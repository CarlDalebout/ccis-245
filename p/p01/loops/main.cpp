...
*****************************************************************************/
const int ALIEN_IN_FLEET = 0;
const int ALIEN_ATTACK = 1;
const int ALIEN_RETURN = 2;

class Alien 
{
public:
    Alien(Surface * psurface)
        :x_(W/2), y_(200), r_(10), dx_(2), dy_(0), speed(3), psurface_(psurface), q_(0)
    {}

    void run(KeyPressed keypressed, int spaceship)
    {
        switch(q_)
        {
            case ALIEN_IN_FLEET:
            ...
            case ALIEN_ATTACK:
            {
                ...
                if(attack_x > spaceship)
                    attack_x -=  speed;
                else
                    attack_x += speed;
                std::cout << attack_x << '\n';
                x_ =  50 * sin(y_ * 0.1) + attack_x;
            }
            break;
            case ALIEN_RETURN:
            {
    ...
    void draw()
    {
        psurface_->put_circle(x_, y_, r_, 255, 255, 255);
    }
    ...
    int dattack_x, dattack_y;
    int y_;
    int r_;
    int dx_, dy_, speed;
    Surface *psurface_;
    int q_; // 0 = in the fleet 1 = attacking 2 = returning
};

class Spaceship 
{
public:
    Spaceship(Surface * psurface)
        :x_(W/2), y_(H - 50), r_(10), dx_(2), dy_(0), psurface_(psurface), q_(0)
    {}

    void run(KeyPressed keypressed)
    {
        x_ += dx_;
    ...
    void draw()
    {
        psurface_->put_circle(x_, y_, r_, 255, 0, 0);
    }
...
void test_circle()
{
	...
	while (1)
	{
		if (event.poll() && event.type() == QUIT) break;
                ...
                //moves
                alien.run(keypressed, spaceship.x_);
                spaceship.run(keypressed);
